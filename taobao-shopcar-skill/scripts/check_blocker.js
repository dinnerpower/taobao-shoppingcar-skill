...
function() {
  ...
  // 检测滑块验证码（排除视频播放器进度条等非验证码元素）
  const slider = document.querySelector("[class*=baxia], [id*=nc_1], [class*=nc-container], [class*=slidetounlock], [class*=slide-verify], [class*=nc-lang-cnt]");
  if (slider && slider.offsetHeight > 0) {
    // 进一步确认：检查是否在验证码弹窗或 overlay 内
    const parent = slider.closest("[class*=dialog], [class*=modal], [class*=overlay], [class*=mask], [class*=baxia]");
    if (parent || slider.id.includes("nc_1") || slider.className.includes("baxia")) {
